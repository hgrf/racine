...
def deleted_sample_handler(session, sample):
    def recursive(sample):
        ...
        session.execute(Share.__table__.update().values(mountpoint_id=0).where(Share.mountpoint_id == sample.id))
...
def deleted_share_handler(session, share):
    def recursive(sample):
        ...
        session.execute(Share.__table__.update().values(mountpoint_id=0)
